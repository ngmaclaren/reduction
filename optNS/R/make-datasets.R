## Write each node set type into a separate function, then make_dataset() basically just dispatches (if that's the right word).

make_dataset <- function(ntrials, ns.type, ...) {
    ns.type <- match.arg() # don't remember how to do this

    ## then call the right function
    ## with(list(...), foo)
    ## probably call mclapply() here
    ## should return a list of list(vs, error, ks[, ws])
}
