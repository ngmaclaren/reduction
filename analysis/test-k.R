...
allk <- apply(conds, 1, function(row) get_ns_k(row[1], row[2]), simplify = FALSE)

df <- do.call(rbind, allk)
...
hist(log(subset(df, network == "ba")$k))

model.glm <- glm(
    k ~ dynamics + network + ns.type, family = gaussian("log"),
...
newdf <- data.frame(
    expand.grid(empiricals, levels(df$dynamics), c("opt", "rand"))
)
colnames(newdf) <- c("network", "dynamics", "ns.type")

newdf$k <- predict(model.glmint, newdata = newdf)

newdfr <- reshape(newdf, timevar = "ns.type", idvar = c("network", "dynamics"), direction = "wide")
newdfr[order(newdfr$network), ]

model.aov <- aov(
    log(k) ~ network + dynamics + ns.type,
