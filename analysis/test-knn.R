...
allknn <- apply(conds, 1, function(row) get_ns_knn(row[1], row[2]), simplify = FALSE)

df <- do.call(rbind, allknn)
...
hist(log(subset(df, network == "ba")$knn))

model.glm <- glm(
    knn ~ network + dynamics + ns.type, family = gaussian("log"),
    data = subset(df, network %in% empiricals & ns.type %in% c("rand", "opt"))
)
model.glmint <- glm(
    knn ~ dynamics + network * ns.type, family = gaussian("log"),
    data = subset(df, network %in% empiricals & ns.type %in% c("rand", "opt"))
)
summary(model.glm)
...
model.aov <- aov(
    log(knn) ~ network + dynamics + ns.type,
