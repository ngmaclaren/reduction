...
for(i in 1:5) {#seq_along(histdat)) {
    ...
    if(i == 1) {
        ...
        barplot(histdat[[i]], names.arg = kposs, col = 1:nrow(histdat[[i]]), # /max(histdat[[i]])
                beside = TRUE, cex.axis = ticksize, cex.names = ticksize, cex.lab = labelsize,
                xlab = "Degree", ylab = "Frequency")
    }
    mtext(LETTERS[i], line = 0, adj = 0.005, font = 2, cex = labelsize)
}
...
points(1:maxn, colMeans(rerrs), col = 2, pch = 19, cex = 2)
segments(
    x0 = 1:maxn,
